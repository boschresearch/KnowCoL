...
def evaluate_oven(output: List[Dict[str, Any]]) -> Dict[str, float]:
    """
    ...
    for item in output:
        ...
        ref_ent_id = item["entity_id"]
        pred_ent_id = item["pred_entity_id"]
...
def evaluate_oven_topk(output: List[Dict[str, Any]]) -> Dict[str, float]:
    """
    Evaluate the predicted results against the reference.
    ...
    for item in output:
        ...
        match_score = int(ref_ent_id in pred_ent_id)
        split2res[data_split].append(match_score)
...
def evaluate_oven_full(output_query: List[Dict[str, Any]], output_entity: List[Dict[str, Any]]) -> Dict[str, Union[float, Dict[str, float]]]:
    """
    ...
    """
    validate_prediction_inputs(output_query)
    validate_prediction_inputs(output_entity)

    #  prepare qid2example
    query_result = evaluate_oven(output_query)
    entity_result = evaluate_oven(output_entity)
...
def evaluate_oven_full_topk(output_query: List[Dict[str, Any]], output_entity: List[Dict[str, Any]]) -> Dict[str, Union[float, Dict[str, float]]]:
    """
    Calculate the final result based on both query and entity results.
    ...
    #  prepare qid2example
    query_result = evaluate_oven_topk(output_query)
    entity_result = evaluate_oven_topk(output_entity)
