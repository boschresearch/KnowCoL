...
chk_dir = '/home/zho1rng/oven_train/checkpoints/2025-01-23/19-23-37/saved_models/epoch=3.ckpt'
model = 'KnowCoL'
device = 'cuda' # cuda
n_px = 224
K = 10
...
with torch.no_grad():
    for batch in tqdm(test_loader_entity):
        entity_ix = model.inference_topk(batch, entity_embs, K) # (B, )
        for i in range(entity_ix.shape[0]):
             output_entity.append({
                  "data_id": batch['data_id'][i], 
                  "pred_entity_id": [kg.entities[j] for j in entity_ix[i]], 
...
evaluate_oven_full(output_entity, output_query)
