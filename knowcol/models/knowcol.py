...
class KnowCoL(pl.LightningModule):
    def __init__(self, encoder: DictConfig, kge: DictConfig, optimizer: DictConfig, beta1: float, beta2: float, temperature: float, kg: KG, lr_scheduler: DictConfig=None, margin=1.0):
    ...
    #         loss += torch.sum(torch.clamp(pos_masks[:,0,:] * logits[:,0,:] - neg_masks[:,i+1,:] * logits[:,i+1,:], max=self.margin/self.temperature)) / torch.sum(pos_masks[:,0,:]) 
    ...
    def training_step(self, batch):
        ...
        img_embs, text_embs = self.clip_encoder(images, text_querys) # (B, N_z)
        ans_embs = img_embs + text_embs
    ...
    def on_test_start(self):
        ...
        for i in tqdm(range(self.kg.n_ent)):
            with torch.no_grad():
                ...
                entity_image_emb = self.clip_encoder.encode_image(entity_image) # (B, N_z)
                entity_text_emb = self.clip_encoder.encode_text(entity_summary_tokenized) # (B, N_z)
    ...
    def inference(self, batch):
        ...
        img_embs, text_embs = self.clip_encoder(images, text_querys) # (B, N_z)
        ans_embs = img_embs + text_embs # (B, n_z)
