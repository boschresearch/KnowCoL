...
class CLIPWrapper(nn.Module):
    ...
    def forward(self, imgs, texts):
        ...
        CLS = torch.zeros(img_embs.shape).to(img_embs)
        cat_emb = torch.stack((CLS, img_embs, text_embs), dim=1) # (B, 3, N_z)
        cat_emb = cat_emb.permute(1, 0, 2) # (3, B, N_z)
        return self.transformer_encoder(cat_emb)[0,:,:] # use the first CLS token embedding as the global embedding
